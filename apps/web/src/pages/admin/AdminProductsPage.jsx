...
const emptyForm = { title: "", slug: "", shortDescription: "", longDescription: "", price: "", oldPrice: "", image: "", gallery: "", sourceUrl: "", documents: "", specifications: "", translations: "", category: "", active: true, featured: false, sortOrder: 0 };
...
export function AdminProductsPage() {
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({ ...emptyForm, ...item, price: item.price ?? "", oldPrice: item.oldPrice ?? "", longDescription: item.longDescription || "", image: item.image || "", sourceUrl: item.sourceUrl || "", category: item.category || "", gallery: (item.images || []).map(({ url }) => url).join("\n"), documents: item.documents ? JSON.stringify(item.documents, null, 2) : "", specifications: item.specifications ? JSON.stringify(item.specifications, null, 2) : "", translations: item.translations ? JSON.stringify(item.translations, null, 2) : "" });
    setModalOpen(true);
  };
  ...
  const save = async (event) => {
    ...
    try {
      ...
      const payload = { ...form, sortOrder: Number(form.sortOrder), price: form.price === "" ? null : Number(form.price), oldPrice: form.oldPrice === "" ? null : Number(form.oldPrice), gallery: form.gallery.split(/\r?\n/).map((url) => url.trim()).filter(Boolean), documents: parseJson(form.documents), specifications: parseJson(form.specifications), translations: parseJson(form.translations) };
      ...
      const message = requestError instanceof SyntaxError ? "Hujjat, xususiyat yoki tarjima JSON formatida xato bor." : requestError.message;
      setError(message);
      notify({ tone: "danger", title: "Mahsulot saqlanmadi", message });
    } finally {
      setSaving(false);
    }
  };
