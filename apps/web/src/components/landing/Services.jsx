...
export function Services({ products, loading, error, onBuy }) {
  ...
  return <section id="products" className="section products-section"><div className="container product-showcase">
    ...
    {!loading && !error && products.length ? <div className="product-grid" ref={trackRef}>{products.map((sourceProduct, index) => { const product = localizeProduct(sourceProduct, language); return <Reveal className={`product-card ${product.image ? "has-catalog-media" : ""} ${index === 0 ? "is-featured" : ""}`} delay={index * .05} key={product.id}>
      <div className="product-accent"/><div className="product-index">{String(index + 1).padStart(2, "0")}</div>{product.image ? <div className="product-catalog-media"><img src={product.image} alt="" loading="lazy"/></div> : null}<div className="product-card-copy"><small>{product.category || "XION PRODUCT"}</small><h3>{product.title}</h3><p>{product.shortDescription}</p><ul>{t.productBenefits.map((benefit) => <li key={benefit}>{benefit}</li>)}</ul><div className="product-price"><strong>{money(product.price, t.priceOnRequest, language, t.currency)}</strong>{product.oldPrice ? <del>{money(product.oldPrice, t.priceOnRequest, language, t.currency)}</del> : null}</div></div>
