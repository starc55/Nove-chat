...
export function Approach({ settings = {}, loading = false }) {
  ...
  return <section className="section news-section" data-scroll><div className="container news-panel">
    ...
    <aside className="support-card" data-scroll data-scroll-speed="0.06"><div><span><Headphones/></span><p>{t.supportEyebrow}</p><h2>{t.supportTitle}</h2></div><div className={`support-details ${loading ? "is-loading" : ""}`}>{loading ? <><span/><span/></> : <>{contact.address ? <p><MapPin size={18}/>{contact.address}</p> : null}{contact.workingHours ? <p><Clock3 size={18}/>{contact.workingHours}</p> : null}</>}</div><a href="#contact">{t.supportCta} <ArrowRight size={17}/></a></aside>
  </div></section>;
}
