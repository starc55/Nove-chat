...
export function Approach({ newsItems = [], loading = false }) {
  ...
  return <section className="section news-section"><div className="container news-panel">
    <div className="news-list"><div className="news-tabs"><strong>{t.news}</strong><span>{t.xionLife}</span></div>{entries.map((item, index) => <Reveal className="news-row" key={item.id || item.slug || item.title} delay={index * .04}><div><time>{new Intl.DateTimeFormat(locale, { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date(item.startAt || item.createdAt || Date.now()))}</time><h3>{item.title}</h3></div><ArrowRight size={18}/></Reveal>)}</div>
