...
export function Footer({ settings = {} }) {
  ...
  const emails = contact.emails?.length ? contact.emails : [{ label: "", value: contact.email || "info@xion.uz" }];
  return <footer id="contact" className="site-footer"><div className="container footer-main">
    <div className="footer-brand"><a className="wordmark" href="/" aria-label={`XION ${t.homeLabel}`}><BrandLogo /></a><p>{settings.company?.descriptor || t.companyDescriptor}</p><div className="footer-socials"><a href={contact.telegramUrl || "https://t.me"} aria-label="Telegram"><Send size={16}/></a></div></div>
