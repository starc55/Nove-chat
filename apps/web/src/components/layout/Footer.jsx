...
export function Footer({ settings = {}, loading = false }) {
  ...
  return <footer id="contact" className="site-footer"><div className="container footer-main">
    ...
    <div className={`footer-contact ${loading ? "is-loading" : ""}`}><div className="footer-socials"><a href="https://instagram.com" target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram size={19}/></a><a href={contact.telegramUrl || "https://t.me/xionuzb"} target="_blank" rel="noreferrer" aria-label="Telegram"><Send size={18}/></a></div><strong>{t.footerSupport}</strong>{loading ? <><span className="footer-skeleton"/><span className="footer-skeleton is-short"/><span className="footer-skeleton"/></> : <>{phones.map((phone) => <a className="footer-phone" key={phone} href={`tel:${phone.replace(/\s/g, "")}`}><Phone size={20}/>{phone}</a>)}{emails.map((email) => <a key={email.value} href={`mailto:${email.value}`} title={email.label || undefined}><Mail size={16}/>{email.value}</a>)}<p><MapPin size={18}/>{contact.address || t.supportAddress}</p>{contact.workingHours ? <small>{contact.workingHours}</small> : null}</>}</div>
